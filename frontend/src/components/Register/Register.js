...
const confirm = (val, all) => {
	...
	if (all.password !== all.confirm) return 'Both passwords should be the same!'
...
const validationFns = {
	...
	userName: [
		val => validator.isAlphanumeric(val) || 'User name should be alphanumeric!'
	],
	email: [
...
const Register = ({history}) => {
	...
	const errorsForField = useCallback(field => {
	...
	const handleSubmit = (e) => {
		...
		Object.keys(errors).length === 0 && register(userInfo)
